...
internal static class VolunteerManager
{
    ...
    internal static void ValidateInputFormat(BO.Volunteer boVolunteer)
    {
        ...
        if (boVolunteer?.Password?.Length < 6 || !IsPasswordStrong(boVolunteer?.Password!))
            throw new BO.BlInvalidInputException("Password is too weak.");
    }
    ...
    {
        if ((BO.Role)original.Role != boVolunteer.Role && boVolunteer.Role != BO.Role.Manager)
            return false;
    ...
    public static BO.CallStatusInProgress CalculateStatus(DO.Call call, int riskThreshold = 30)
    {
        ...
        return timeToEnd?.TotalMinutes <= riskThreshold
            ? BO.CallStatusInProgress.AtRisk
            : BO.CallStatusInProgress.InProgress;
    }
